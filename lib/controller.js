...
class Controller {
  ...
  find(req, res, next) {
    return this.model.find(req.query)
        .then(collection => {
            // return 404 if bookshelf is empty, if not return json of collection
  ...
  //   return this.model.find({title: req.params.title})
  //   .then(doc => {
  //     "use strict";
  //     return doc.length > 0 ? res.status(200).json(doc) : res.status(404).send({message: "book not found"});
  //   })
  // }
  ...
  update(req, res, next) {
    ...
    if (req.body.userType && ['Reader', 'Librarian'].indexOf(req.body.userType) === -1) {
